...
from simulation_financiere import formater_resultat_optimisation, creer_donnees_graphique_waterfall
...
def afficher_sidebar_parametres():
    ...
    with st.sidebar:
        ...
        plafond_per_annuel = st.number_input(
            ...
            min_value=0.0,
            max_value=50000.0,
            value=st.session_state.params['plafond_per_annuel'],
            step=100.0
        )
        ...
        with col_capital:
            ...
            with col_check_cap:
                activer_capital = st.checkbox(
                    ...
                    value=st.session_state.activer_vars.get(actif['capital_key'], actif['capital_default']),
                    help="Optimiser ce paramètre"
                )
            
            with col_val_cap:
                if activer_capital:
                    capital_value = st.session_state.current_values[actif['capital_key']]
                    ...
                    capital_value = st.number_input(
                        ...
                        min_value=0.0,
                        max_value=100000.0,
                        value=st.session_state.current_values[actif['capital_key']],
                        step=1000.0,
                        key=f"{actif['capital_key']}_fixe",
                        label_visibility="collapsed",
                        format="%.0f"
                    )
                    st.session_state.current_values[actif['capital_key']] = capital_value
        ...
        with col_versement:
            ...
            with col_check_vers:
                activer_versement = st.checkbox(
                    ...
                    value=st.session_state.activer_vars.get(actif['versement_key'], actif['versement_default']),
                    help="Optimiser ce paramètre"
                )
            
            with col_val_vers:
                if activer_versement:
                    versement_value = st.session_state.current_values[actif['versement_key']]
                    ...
                    versement_value = st.number_input(
                        ...
                        min_value=0.0,
                        max_value=2000.0,
                        value=st.session_state.current_values[actif['versement_key']],
                        step=50.0,
                        key=f"{actif['versement_key']}_fixe",
                        label_visibility="collapsed",
                        format="%.0f"
                    )
                    st.session_state.current_values[actif['versement_key']] = versement_value
    ...
    with col_credit:
        ...
        with col_check_credit:
            activer_credit_scpi = st.checkbox(
                ...
                value=st.session_state.activer_vars.get('credit_scpi_montant', True),
                help="Optimiser le montant du crédit SCPI"
            )
        
        with col_val_credit:
            if activer_credit_scpi:
                credit_scpi_montant = st.session_state.current_values['credit_scpi_montant']
                ...
                credit_scpi_montant = st.number_input(
                    ...
                    min_value=0.0,
                    max_value=500000.0,
                    value=st.session_state.current_values['credit_scpi_montant'],
                    step=5000.0,
                    key="credit_scpi_montant_fixe",
                    label_visibility="collapsed",
                    format="%.0f"
                )
                st.session_state.current_values['credit_scpi_montant'] = credit_scpi_montant
    ...
    with st.expander("⚙️ Paramètres avancés de simulation", expanded=False):
        ...
        with col1:
            ...
            duree_annees = st.number_input(
                ...
                min_value=1.0,
                max_value=50.0,
                value=st.session_state.params.get('duree_annees', 20.0),
                step=1.0
            )
            ...
            credit_scpi_duree = st.number_input(
                ...
                min_value=1.0,
                max_value=30.0,
                value=st.session_state.params.get('credit_scpi_duree', 15.0),
                step=1.0
            )
            ...
            taux_av = st.number_input(
                ...
                min_value=0.0,
                max_value=20.0,
                value=st.session_state.params.get('taux_av', 0.04) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            taux_per = st.number_input(
                ...
                min_value=0.0,
                max_value=20.0,
                value=st.session_state.params.get('taux_per', 0.05) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            taux_distribution_scpi = st.number_input(
                ...
                min_value=0.0,
                max_value=15.0,
                value=st.session_state.params.get('taux_distribution_scpi', 0.045) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            taux_appreciation_scpi = st.number_input(
                ...
                min_value=0.0,
                max_value=10.0,
                value=st.session_state.params.get('taux_appreciation_scpi', 0.02) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
        ...
        with col2:
            ...
            frais_entree_av = st.number_input(
                ...
                min_value=0.0,
                max_value=10.0,
                value=st.session_state.params.get('frais_entree_av', 0.03) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            frais_entree_per = st.number_input(
                ...
                min_value=0.0,
                max_value=10.0,
                value=st.session_state.params.get('frais_entree_per', 0.025) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            frais_entree_scpi = st.number_input(
                ...
                min_value=0.0,
                max_value=20.0,
                value=st.session_state.params.get('frais_entree_scpi', 0.10) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            credit_scpi_taux = st.number_input(
                ...
                min_value=0.0,
                max_value=10.0,
                value=st.session_state.params.get('credit_scpi_taux', 0.035) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            credit_scpi_assurance = st.number_input(
                ...
                min_value=0.0,
                max_value=2.0,
                value=st.session_state.params.get('credit_scpi_assurance', 0.003) * 100,
                step=0.1,
                format="%.2f"
            ) / 100
            ...
            tmi = st.number_input(
                ...
                min_value=0.0,
                max_value=50.0,
                value=st.session_state.params.get('tmi', 0.30) * 100,
                step=1.0,
                format="%.1f"
            ) / 100
            ...
            plafond_per_annuel = st.number_input(
                ...
                min_value=0.0,
                max_value=100000.0,
                value=st.session_state.params.get('plafond_per_annuel', 35000.0),
                step=1000.0
            )
            ...
            scpi_europeenne_ratio = st.number_input(
                ...
                min_value=0.0,
                max_value=100.0,
                value=st.session_state.params.get('scpi_europeenne_ratio', 0.0) * 100,
                step=5.0,
                format="%.1f"
            ) / 100
        ...
        st.session_state.params.update(parametres_mis_a_jour)
