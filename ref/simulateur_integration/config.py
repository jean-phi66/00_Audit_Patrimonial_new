...
from simulation_financiere import creer_parametres_defaut
...
def initialiser_session_state():
    ...
    if 'params_initialized' not in st.session_state:
        ...
        st.session_state.params = creer_parametres_defaut()
...
def mettre_a_jour_parametres_sidebar(parametres_sidebar: Dict[str, Any]):
    ...
    if parametres_sidebar['plafond_per_annuel'] != st.session_state.params['plafond_per_annuel']:
        st.session_state.params['plafond_per_annuel'] = parametres_sidebar['plafond_per_annuel']
...
def mettre_a_jour_variables_optimisation(variables_info: Dict[str, Dict[str, Any]]):
    ...
    for var_name, info in variables_info.items():
        ...
        st.session_state.activer_vars[var_name] = info['activer']
        
        # Mise à jour de la valeur courante
        st.session_state.current_values[var_name] = info['valeur']
...
def preparer_parametres_optimisation(parametres_sidebar: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for var_name in variables_ordre:
        is_active = st.session_state.activer_vars.get(var_name, False)
        activer_vars.append(is_active)
        valeurs_defaut.append(st.session_state.current_values.get(var_name, 0.0))
    
    # Valeurs fixées (non optimisées) - pour information
    valeurs_fixes = {}
    for var_name in variables_ordre:
        if not st.session_state.activer_vars.get(var_name, False):
            valeurs_fixes[var_name] = st.session_state.current_values.get(var_name, 0.0)
...
def mettre_a_jour_valeurs_courantes_avec_resultat(resultat: Dict[str, Any]):
    ...
    for cle_resultat, cle_session in mapping_cles.items():
        if cle_resultat in resultat:
            st.session_state.current_values[cle_session] = resultat[cle_resultat]
...
def obtenir_parametres_interface() -> Dict[str, Any]:
    ...
    return {
        ...
        'activer_vars': st.session_state.activer_vars,
        'dernier_resultat': st.session_state.dernier_resultat
    }
...
def valider_coherence_parametres() -> tuple[bool, list[str]]:
    ...
    au_moins_une_variable = any(st.session_state.activer_vars.values())
    ...
    versement_per_annuel = st.session_state.current_values['versement_per'] * 12
    if versement_per_annuel > st.session_state.params['plafond_per_annuel']:
        erreurs.append(f"Les versements PER annuels ({versement_per_annuel:,.0f} €) dépassent le plafond autorisé ({st.session_state.params['plafond_per_annuel']:,.0f} €)")
...
def obtenir_resume_configuration() -> Dict[str, Any]:
    ...
    """
    variables_activees = [k for k, v in st.session_state.activer_vars.items() if v]
    variables_fixees = [k for k, v in st.session_state.activer_vars.items() if not v]
    ...
    return {
        ...
        'plafond_per_annuel': st.session_state.params['plafond_per_annuel'],
        'duree_simulation': st.session_state.params['duree_annees']
    }